...
5. Добавьте Environment Variables:
   ...
   - `CORS_ORIGIN` - для CORS настроек (без завершающего слэша!)
   - `FRONTEND_URL` - для генерации ссылок верификации email (без завершающего слэша!)
   - Используйте ваш реальный frontend URL из Vercel (после деплоя frontend, см. Шаг 3)
...
5. Добавьте Environment Variable:
   ...
   **Если вы видите ошибку "references Secret which does not exist":**

   **Шаг 1: Удалите существующую переменную (если есть):**

   **Где найти Environment Variables в Vercel:**

   - Откройте ваш проект в Vercel Dashboard
   - В верхнем меню проекта найдите вкладку **"Settings"** (или иконку шестеренки ⚙️)
   - Или в боковом меню слева найдите **"Settings"**
   - Внутри Settings найдите раздел **"Environment Variables"** (или "Variables")
   - Или попробуйте: в боковом меню проекта найдите **"Environment Variables"** напрямую

   - Найдите переменную `VITE_API_BASE` в списке
   - Если она есть, нажмите на иконку корзины (🗑️) справа, чтобы удалить
   - Подтвердите удаление

   **Шаг 2: Добавьте новую переменную:**

   - В разделе "Environment Variables" нажмите кнопку **"Add New"** (или **"Add Variable"**, или **"Create"**)
   - В поле **"Key"** (или **"Name"**) введите: `VITE_API_BASE`
   - В поле **"Value"** вставьте **реальный URL вашего backend** из Render
     - Например: `https://moonsalon-backend.onrender.com` (ваш реальный URL!)
   - ⚠️ **НЕ используйте примеры** - используйте ваш реальный URL из Render Dashboard!
   - Просто введите значение напрямую в поле "Value" - не создавайте Secret!
   - Нажмите **"Save"** (или **"Add"**, или **"Create"**)
...
6. Вставьте его в значение `CORS_ORIGIN`, заменив placeholder:
   ...
   - **НЕ добавляйте завершающий слэш** в конце URL!
   - Используйте: `https://dragon-1-ten.vercel.app` ✅
   - НЕ используйте: `https://dragon-1-ten.vercel.app/` ❌ (с завершающим слэшем)
...
7. **Также добавьте переменную `FRONTEND_URL`** (для ссылок верификации email):

   - В том же разделе Environment нажмите "Add Environment Variable" (или "Add New")
   - Key: `FRONTEND_URL`
   - Value: тот же URL, что и в `CORS_ORIGIN` (без завершающего слэша!)
   - Например: `https://dragon-1-ten.vercel.app` (ваш реальный URL!)
   - ⚠️ **НЕ добавляйте завершающий слэш** в конце URL!
   - Нажмите "Save"

8. Нажмите "Save Changes" - сервис автоматически перезапустится
...
### Проблема: Frontend показывает ошибки "Failed to fetch"
...
Эта ошибка означает, что frontend не может подключиться к backend. Проверьте следующее:

**1. Проверьте, что `VITE_API_BASE` настроен правильно в Vercel:**
...
- Найдите переменную `VITE_API_BASE` в списке
- Проверьте:
  - ✅ Значение должно быть: `https://moonsalon-backend.onrender.com` (ваш реальный URL!)
  - ✅ В колонке "Environments" должна быть галочка для **Production** (обязательно!)
  - ⚠️ **НЕ используйте `http://localhost:8080`** - это только для локальной разработки!

**Шаг 2: Если переменная есть, но не работает:**

- Удалите переменную (иконка корзины 🗑️)
- Добавьте заново:
  - Key: `VITE_API_BASE`
  - Value: `https://moonsalon-backend.onrender.com` (ваш реальный URL!)
  - Environments: ✅ Production (обязательно!), ✅ Preview, ✅ Development (опционально)
  - Нажмите "Save"

**Шаг 3: Пересоберите проект (ОБЯЗАТЕЛЬНО!):**

- В Vercel Dashboard: Deployments → выберите последний деплой → "Redeploy"
- Или запушьте новый коммит в GitHub
- ⚠️ **Важно:** В Vite переменные окружения встраиваются во время сборки, поэтому после изменения переменной нужно пересобрать проект!

**2. Проверьте, что backend доступен:**

- Откройте в браузере: `https://<ваш-backend-url>.onrender.com/api/health`
- Должен вернуться: `{"status":"UP","service":"backend"}`
- Если не работает - проверьте логи в Render Dashboard

**3. Проверьте CORS в backend:**

- Откройте Render Dashboard → ваш backend сервис → Environment
- Найдите переменную `CORS_ORIGIN`
- Убедитесь, что она указывает на ваш реальный frontend URL из Vercel
- Например: `https://moonsalon-abc123.vercel.app` (ваш реальный URL!)
- ⚠️ **НЕ используйте `http://localhost:5173`** - это только для локальной разработки!

**4. Пересоберите frontend после изменения переменных:**

- В Vercel Dashboard: Deployments → выберите последний деплой → "Redeploy"
- Или просто запушьте новый коммит в GitHub

**5. Проверьте консоль браузера:**

- Откройте DevTools (F12) → вкладка "Console"
- Посмотрите на точную ошибку - там может быть больше информации
- Проверьте вкладку "Network" - какие запросы отправляются и какие ошибки возвращаются
...
1. **Удалите существующую переменную:**
   ...
   **Где найти Environment Variables в Vercel:**
   ...
   - Откройте ваш проект в Vercel Dashboard
   - В боковом меню проекта (слева) найдите **"Settings"**
   - Внутри Settings найдите раздел **"Environment Variables"** (или просто **"Variables"**)
   - Или в верхнем меню проекта найдите вкладку **"Settings"** → **"Environment Variables"**

   - Найдите переменную `VITE_API_BASE` в списке
   - Если она существует, удалите её (нажмите на иконку корзины 🗑️)

2. **Добавьте новую переменную правильно:**

   - Нажмите "Add New" (или "Add Variable", или "Create")
   - В поле "Key" (или "Name") введите: `VITE_API_BASE`
   - В поле "Value" вставьте **реальный URL вашего backend** из Render
     - Например: `https://moonsalon-backend.onrender.com` (ваш реальный URL!)
   - ⚠️ **НЕ используйте примеры** - используйте ваш реальный URL из Render Dashboard!
   - В разделе **"Environments"** (или **"Environment"**) выберите галочки:
     - ✅ Production
     - ✅ Preview (опционально)
...
⚠️ **ВАЖНО:** Если Root Directory установлен как `frontend`, файл `vercel.json` должен находиться **в папке `frontend`**, а не в корне проекта!

- Убедитесь, что в папке `frontend` есть файл `vercel.json` с правильной конфигурацией:
...
- Если `vercel.json` находится в корне проекта, переместите его в папку `frontend`
- Или создайте новый `vercel.json` в папке `frontend` с конфигурацией выше
